# Infrastructure code for "VW MAN Data Lake - CONSUMPTION Architecture"
...
Infrastructure consists of multiple layers (s3_-_enriched_zonepartitioned_by_county, dynamodb_-_index_by_county) where each layer is described using one [Terraform module](https://www.terraform.io/docs/configuration/modules.html) with `inputs` arguments specified in `terragrunt.hcl` in respective layer's directory.
...
- **As a single layer (faster&granular).** Run this command to create infrastructure in a single layer (eg, `s3_-_enriched_zonepartitioned_by_county`):

```
$ cd eu-west-1/s3_-_enriched_zonepartitioned_by_county
$ terragrunt apply
```
